...
export default function WizardPage() {
  ...
  const fieldClass =
    "w-full rounded-xl border border-gray-300 px-3.5 py-3 text-base sm:rounded-lg sm:px-3 sm:py-2 sm:text-sm";

  return (
    <div className="min-h-[100dvh] bg-gray-50 p-0 sm:p-8">
      <form
        onSubmit={save}
        className="mx-auto max-w-3xl space-y-5 border-gray-200 bg-white p-5 sm:rounded-xl sm:border sm:p-8 sm:shadow-sm"
      >
        <div className="pt-safe">
          <h1 className="page-title text-2xl font-bold">पहला सेटअप</h1>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <input required value={publisherName} onChange={(e) => setPublisherName(e.target.value)} placeholder="पब्लिशर / संपादक का नाम" autoComplete="name" className={fieldClass} />
          <input required value={newspaperName} onChange={(e) => setNewspaperName(e.target.value)} placeholder="अखबार का नाम" className={fieldClass} />
          <select value={pubType} onChange={(e) => setPubType(e.target.value)} className={fieldClass}>
            <option value="Daily">दैनिक</option>
            <option value="Weekly">साप्ताहिक</option>
          </select>
          <select value={pages} onChange={(e) => setPages(e.target.value)} className={fieldClass}>
            <option value="6">6 पेज</option>
            <option value="8">8 पेज</option>
            <option value="12">12 पेज</option>
            <option value="24">24 पेज</option>
          </select>
          <input value={city} onChange={(e) => setCity(e.target.value)} placeholder="शहर (मास्टहेड पर छपेगा)" autoComplete="address-level2" className={fieldClass} />
          <input value={coverPrice} onChange={(e) => setCoverPrice(e.target.value)} placeholder="अंक मूल्य (जैसे Rs. 5)" className={fieldClass} />
          <input
            value={startYear}
          ...
          <label className="tap-row flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-dashed border-gray-300 px-3.5 py-4 text-sm sm:col-span-2">
            <span className="min-w-0 truncate">फ्रंट पेज हेडर इमेज {frontHeaderName ? `— ${frontHeaderName}` : ""}</span>
            <span className="shrink-0 text-xs font-semibold text-gray-600">फाइल चुनें</span>
            <input type="file" accept="image/*" hidden onChange={(e) => void handleHeaderUpload(e.target.files?.[0], "front")} />
          </label>
          <label className="tap-row flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-dashed border-gray-300 px-3.5 py-4 text-sm sm:col-span-2">
            <span className="min-w-0 truncate">इनसाइड पेज हेडर इमेज {insideHeaderName ? `— ${insideHeaderName}` : ""}</span>
            <span className="shrink-0 text-xs font-semibold text-gray-600">फाइल चुनें</span>
            <input type="file" accept="image/*" hidden onChange={(e) => void handleHeaderUpload(e.target.files?.[0], "inside")} />
          </label>
        </div>
        {/* Sticky on phones: this form is long enough that the submit button
            would otherwise sit far below the fold behind the keyboard. */}
        <div className="sticky bottom-0 -mx-5 border-t border-gray-200 bg-white px-5 py-4 pb-safe sm:static sm:mx-0 sm:border-0 sm:bg-transparent sm:p-0">
          <button
            disabled={saving}
            className="tap w-full btn-brand rounded-xl py-3.5 text-base font-semibold disabled:opacity-50 sm:w-auto sm:rounded-lg sm:px-5 sm:py-2.5 sm:text-sm"
          >
            {saving ? "सेव हो रहा है..." : "सेटअप पूरा करें"}
          </button>
        </div>
      </form>
    </div>
